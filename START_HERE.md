...
3. Double-click **RUN_TRAINING.cmd**. Keep its window open while it runs. The first run may download pretrained MobileNetV2 weights. A successful V2 run creates `artifacts/waste_classifier_int8.tflite`, `artifacts/labels.txt`, `artifacts/open_set.json`, and measured results in `artifacts/training_summary.json`.
4. Double-click **RUN_DEMO.cmd**. Keep one item inside the green square, hold it steady, then press **SPACE** to analyze 7 frames. Press **Q** to close. This is a dry-run: it displays recognition and routing decisions without moving lids. The material prediction is accepted only when the validity, feature-distance, confidence, margin, and multi-frame checks pass.
